...
def offset_cal(points,opt):
	...
	#points: B*(21+1024)*3
        ...
        inputs1_diff = points.transpose(1,2).unsqueeze(1).expand(cur_train_size,opt.JOINT_NUM,3,opt.SAMPLE_NUM+21) \
        			 - points[:,0:opt.JOINT_NUM,:].unsqueeze(-1).expand(cur_train_size,opt.JOINT_NUM,3,opt.SAMPLE_NUM+21) #B*21*3*1045
        inputs1_diff = inputs1_diff[:,:,:,21:opt.SAMPLE_NUM+21] #B*21*3*1024
        ...
        point_cld = points[:,21:opt.SAMPLE_NUM+21,:] 	#points: B*(1024)*3
       ...
       #print(heatmap.shape)
        heatmap = heatmap.contiguous().view(cur_train_size,opt.JOINT_NUM*4,opt.SAMPLE_NUM) #B*4J*1024
